{
  ...
  outputs = { self, nixpkgs }:
    let
      ...
      mkPerSystem = f: nixpkgs.lib.genAttrs systems (system:
        let
...
EOF
        ...
        in f { inherit system pkgs env ratesPkg ratesJsonFile dateFile; ecbRateCli = ecbRateCli; }
      );
    in {
      packages = mkPerSystem ({ ratesPkg, ecbRateCli, ... }: {
        default = ratesPkg;
        rates = ratesPkg;
        cli = ecbRateCli;
      });

      apps = mkPerSystem ({ ecbRateCli, ... }: {
        default = {
          type = "app";
      ...
      checks = mkPerSystem ({ pkgs, ratesPkg, ... }: {
